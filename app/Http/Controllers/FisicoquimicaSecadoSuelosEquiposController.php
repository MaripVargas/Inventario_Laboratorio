...
use App\Exports\FisicoquimicaSecadoSuelosEquiposExport;
...
class FisicoquimicaSecadoSuelosEquiposController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('labs.fisicoquimica.secado_suelos_equipos.index', compact('items'));
    }

    public function create()
    {
        $catalogos = $this->getCatalogos();
        return view('labs.fisicoquimica.secado_suelos_equipos.create', compact('catalogos'));
    }
    ...
    public function store(Request $request)
    {
        ...
        FisicoquimicaSecadoSuelosEquipo::create($request->all());

        return redirect()->route('fisicoquimica.secado_suelos_equipos.index')
                         ->with('success', 'Equipo agregado correctamente.');
    }

    public function edit($id)
    {
        $item = FisicoquimicaSecadoSuelosEquipo::findOrFail($id);
        $catalogos = $this->getCatalogos();
        return view('labs.fisicoquimica.secado_suelos_equipos.edit', compact('item', 'catalogos'));
    }
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('exports.fisicoquimica.secado_suelos_equipos', [
            'items' => $items,
            'generatedAt' => now(),
        ])->setPaper('A4', 'landscape');

        return $pdf->download('fisicoquimica_secado_suelos_equipos_' . now()->format('Y-m-d_His') . '.pdf');
    }
    ...
    public function exportExcel(Request $request)
    {
        ...
        return Excel::download(
            new FisicoquimicaSecadoSuelosEquiposExport($items),
            'fisicoquimica_secado_suelos_equipos_' . now()->format('Y-m-d_His') . '.xlsx'
        );
    }
    ...
    private function getCatalogos()
    {
        ...
        $equipos = FisicoquimicaSecadoSuelosEquipo::select(
            'nombre_responsable',
            'cedula',
            'vinculacion',
            'usuario_registra'
    ...
    {
        return FisicoquimicaSecadoSuelosEquipo::query()
